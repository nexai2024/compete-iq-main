...
export function FeatureMatrix({
  ...
  // Performance optimization: Pre-process scores into a Map for O(1) lookup in the render loop.
  // This reduces complexity from O(Scores * Params * Entities) to O(Scores + Params * Entities).
  const scoreMap = useMemo(() => {
    const map = new Map<string, number>();
    scores.forEach((s) => {
      // Use a composite key to uniquely identify each score cell
      const key = s.entityType === 'user_app'
        ? `user_app-${s.parameterId}`
        : `competitor-${s.entityId}-${s.parameterId}`;
      map.set(key, s.score);
    });
    return map;
  }, [scores]);
